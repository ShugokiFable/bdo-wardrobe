...
import { mergeCatalogs, normalizeCatalog, slugify } from './catalog.mjs';
...
export function parsePearlOutfitGuide(html, pageUrl=PEARL_PREMIUM_GUIDE_URL) {
  ...
  while ((m = labelRe.exec(source))) {
    const name = m[2].replace(/&amp;/gi, '&').replace(/&nbsp;/gi, ' ').replace(/\s+/g, ' ').trim();
    if (!name) continue;
  ...
  for (let redirects = 0; redirects <= 4; redirects++) {
    const response = await fetch(url, {
      ...
      headers:{
        ...
        'referer':url.hostname.endsWith('pearlcdn.com') ? 'https://blackdesert.pearlabyss.com/' : 'https://bdo.mmo-fashion.com/'
      }
    });
    if ([301,302,303,307,308].includes(response.status)) {
      const location = response.headers.get('location');
